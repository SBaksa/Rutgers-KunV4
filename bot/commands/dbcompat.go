...
func DBCompat(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	...
	s.ChannelMessageEditEmbed(m.ChannelID, sentMsg.ID, embed)
}

func DBDebug(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if database.Instance == nil {
		s.ChannelMessageSend(m.ChannelID, "Database not initialized")
		return
	}
